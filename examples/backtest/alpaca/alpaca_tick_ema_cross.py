#!/usr/bin/env python3
# -------------------------------------------------------------------------------------------------
#  Alpaca Tick Data EMA Cross Backtest Example
#
#  This example demonstrates how to run a simple EMA crossover backtest using
#  historical TICK data (trade ticks) from Alpaca API for AAPL.
#
#  Note: Tick data can be very large - this example uses a short time window.
#
#  Prerequisites:
...
if __name__ == "__main__":
    ...
    # Define time range (using a 1-hour window for tick data as it can be very large)
    end_time = datetime(2026, 1, 3, 10, 30, 0)  # 10:30 AM
    start_time = datetime(2026, 1, 3, 9, 30, 0)  # 9:30 AM (market open)

    print(f"Running tick-based backtest for {SYMBOL} on Alpaca")
    print(f"Time range: {start_time} to {end_time}")
    print("-" * 60)

    # -------------------------------------------------------------------------
    # Load historical tick data from Alpaca API
    # -------------------------------------------------------------------------

    print("Loading historical trade tick data from Alpaca API...")
    ...
    if not ticks:
        print("ERROR: No tick data loaded. Check your API keys and ensure market was open.")
        exit(1)
